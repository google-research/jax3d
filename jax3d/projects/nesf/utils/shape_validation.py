...
def assert_typing(fn: _Fn) -> _Fn:
  ...
  ```
  @jax3d.projects.nesf.assert_typing
...
class _ShapeTracker:
  ...
  def track_and_validate_shape(
      self,
      shape: Tuple[int, ...],
      expected_shape: jtyping.ShapeSpec,
  ) -> None:
  ...
  def resolve_spec(self, shape_spec: jtyping.ShapeSpec) -> str:
...
def _assert_type(
  ...
  # Should recurse into `Dict`, `List`, `dataclass`,...
  if not isinstance(annotation, jtyping.ArrayAliasMeta):
    return
  try:
...
def assert_match_array_alias(
    array: jnp.ndarray,
    expected_spec: jtyping.ArrayAliasMeta,
) -> None:
  """Check that a specific value match its annotated type."""
