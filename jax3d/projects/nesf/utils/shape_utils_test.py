...
from jax3d.projects.nesf.typing import f32, i32  # pylint: disable=g-multiple-import
from jax3d.projects.nesf.utils import shape_utils
import numpy as np
import pytest
...
def test_shape_utils():
  ...
  tensor_specs = jax3d.tensor_spec_like(ones + [
  ...
  jax3d.testing.assert_tree_all_equal_spec(zeros, expected_zeros)
  jax3d.testing.assert_tree_all(jnp.allclose, zeros, expected_zeros)
...
def test_shape_dtype_like():
  arr_spec = jax3d.shape_dtype_like([{'a': jnp.zeros((2, 1), dtype=np.int32)}])
  assert repr(arr_spec) == "[{'a': i32[2 1]}]"


def test_type_like():
  arr_spec = jax3d.types_like([{'a': jnp.zeros((2, 1), dtype=np.int32)}])
