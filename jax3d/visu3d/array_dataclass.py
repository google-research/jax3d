...
class DataclassArray:
  """Dataclass which behave like an array.

  Usage:

  ```python
  @dataclasses.dataclass
  class Point(DataclassArray):
    x: Array[''] = array_field(shape=())
    y: Array[''] = array_field(shape=())

  p = Point(x=[1, 2, 3, 4], y=[5, 6, 7, 8])
  p.shape == (4,)
  p[0] == Point(x=1, y=5)
  p.reshape((2, 2)) == Point(x=[[1, 2], [3, 4]], y=[[5, 6], [7, 8]])
  ```
  ...
  def __post_init__(self) -> None:
    ...
    if len(shapes) > 1:
      raise ValueError(f'Conflicting shapes: {shapes}')
